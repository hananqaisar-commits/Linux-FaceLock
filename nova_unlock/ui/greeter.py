...
class Greeter(QWidget):
    ...
    def _paint_bg(self, P):
        if self.dark:
            bg = QRadialGradient(self.W/2, self.H * 0.38, self.W * 1.1)
            bg.setColorAt(0.0, QColor(12, 15, 26))
            bg.setColorAt(0.35, QColor(6, 8, 16))
            bg.setColorAt(0.7,  QColor(2, 3, 8))
        ...
        # Subtle vignette
        v = QRadialGradient(self.W/2, self.H/2, max(self.W, self.H) * 0.65)
        v.setColorAt(0.0, QColor(0, 0, 0, 0))
        v.setColorAt(0.55, QColor(0, 0, 0, 0))
        v.setColorAt(0.85, QColor(0, 0, 0, 55))
        v.setColorAt(1.0,  QColor(0, 0, 0, 120))
        P.setBrush(QBrush(v))
        P.drawRect(self.rect())
