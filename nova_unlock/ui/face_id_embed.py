...
"""
import sys, math, math, time, struct, wave, tempfile, os, subprocess, random
...
def write_pam_cache(username: str, cache_path: str = None) -> None:
    """Write face-auth result to PAM cache for pam_script/pam_exec."""
    import json as _j
    import time as _t
    import os   as _o
    if cache_path is None:
        cache_path = CACHE_FILE
    _u   = str(username).strip().lower()
    data = {"user": _u, "profile": _u, "ts": _t.time()}
    log(f"write_pam_cache: user={_u}")
    with open(cache_path, "w") as _f:
        _j.dump(data, _f)
    try:
        _o.chmod(cache_path, 0o600)
    except OSError:
        pass
...
def ensure_on_top(widget) -> None:
    ...
    try:
        ...
        if wid:
            ...
            _xdotool_result = (r.returncode == 0)
    except Exception:
        _xdotool_result = False
    ...
    try:
        ...
        widget.setWindowFlags(
            widget.windowFlags() | _Qt.WindowStaysOnTopHint
        )
        widget.show()
        widget.raise_()
