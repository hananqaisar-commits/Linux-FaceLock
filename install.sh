...
echo
echo -e "${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║     NovaUnlock — Installer v4.0              ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${NC}"
echo
...
# ═══════════════════════════════════════════════════════════════
# STEP 3 — Python venv + pip packages
# ═══════════════════════════════════════════════════════════════
echo
echo -e "${CYAN}[3/8] Setting up Python environment...${NC}"
...
if [ ! -d "$VENV" ]; then
    ...
    su - "$REAL_USER" -c "python3 -m venv '$VENV'" >>"$INSTALL_LOG" 2>&1 || {
...
if ! command -v cmake >/dev/null 2>&1; then
    ...
    # Run with output visible so user sees progress, log simultaneously
    su - "$REAL_USER" -c "'$VENV/bin/pip' install dlib 2>&1" | tee -a "$INSTALL_LOG"
    DLIB_EXIT="${PIPESTATUS[0]}"

    echo -e "  ${CYAN}→${NC}  [ dlib compile ended:  $(date +%H:%M:%S) ]" | tee -a "$INSTALL_LOG"

    if [ "$DLIB_EXIT" -eq 0 ]; then
        info "Installing face_recognition..."
        su - "$REAL_USER" -c "'$VENV/bin/pip' install -q face_recognition" >>"$INSTALL_LOG" 2>&1 && \
        ok "face_recognition installed" || \
        fail "face_recognition install failed (dlib OK but face_recognition failed)"
    else
        fail "dlib compile failed — check $INSTALL_LOG for details"
        warn "Common fix: ensure libboost-dev, libopenblas-dev, cmake are installed"
    fi
fi
...
#%PAM-1.0
$AUTH_LINE
auth    include       $COMMON_AUTH
account include       $COMMON_AUTH
session include       $COMMON_AUTH
PAMEOF
    fi
    ok "PAM: $label"
}
...
case "$DE" in
    xfce)     configure_pam "/etc/pam.d/xfce4-screensaver"    "xfce4-screensaver" ;;
    gnome)    configure_pam "/etc/pam.d/gnome-screensaver"    "gnome-screensaver"
              configure_pam "/etc/pam.d/gdm-password"         "gdm-password"      ;;
    kde)      configure_pam "/etc/pam.d/kde"                  "kde"
              configure_pam "/etc/pam.d/sddm"                 "sddm"              ;;
    mate)     configure_pam "/etc/pam.d/mate-screensaver"     "mate-screensaver"  ;;
    cinnamon) configure_pam "/etc/pam.d/cinnamon-screensaver" "cinnamon-screensaver" ;;
    *)        configure_pam "/etc/pam.d/xfce4-screensaver"    "xfce4-screensaver"
              configure_pam "/etc/pam.d/gnome-screensaver"    "gnome-screensaver"
...
GREETER_SCRIPT="$NOVA_DIR/scripts/face_login_greeter.py"
HELPER_EXPAND
...
LDMEOF
    ...
    warn "GDM — greeter face login not supported yet (lock screen unlock works)"
elif [ "$DM" = "sddm" ]; then
...
LOCK_UID="$REAL_UID"
NOVA_VENV="$VENV"
NOVA_DAEMON="$NOVA_DIR/scripts/face_unlock_daemon.py"
...
case "\$RUNTIME_DE" in
    xfce)
        pgrep -x xfce4-screensaver >/dev/null 2>&1 && exec xfce4-screensaver-command -l
        pgrep -x light-locker      >/dev/null 2>&1 && exec light-locker-command -l
        ;;
    gnome)
...
case "$DE" in
    xfce)
        ...
        ;;
    gnome)
        warn "GNOME: Bind Super+L → /usr/local/bin/nova_xflock4_lock.sh in Keyboard Shortcuts"
        ;;
    kde)
        warn "KDE: Bind lock shortcut → /usr/local/bin/nova_xflock4_lock.sh in System Settings"
        ;;
    *)
...
DAEMON="$NOVA_DIR/scripts/face_unlock_daemon.py"
