...
def main():
    ...
    print()
    print("╔══════════════════════════════════════════════╗")
    print("║     NovaUnlock — Installer v5.2              ║")
    print("╚══════════════════════════════════════════════╝")
    print()
    ...
    # Fix ownership (group may differ from username on LDAP/custom setups)
    subprocess.run(
    ...
    if result.returncode in (0, 1):
        enroll_script = py_entry(install_dir, "scripts/enroll_gui.py")
...
def launch_enrollment_wizard():
    ...
    if not venv_py.exists() or not setup_flow.exists():
        print(f"[Nova]  Wizard not found — enroll manually:")
        print(f"[Nova]  {venv_py} {setup_flow}")
        return
